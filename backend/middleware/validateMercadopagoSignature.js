...
module.exports = async function validateMercadopagoSignature(req, res, next) {
  ...
  if (!secret || String(secret).trim() === '') {
    logger.error('[MercadoPago Security] Rejection: MERCADOPAGO_WEBHOOK_SECRET not set');
  ...
  if (!signature) {
    logger.warn('[MercadoPago Security Warning] Missing x-signature header');
  ...
  try {
    ...
    if (!ts || !v1) {
      logger.warn('[MercadoPago Security Warning] Malformed x-signature header');
    ...
    if (isNaN(requestTs) || Math.abs(currentTs - requestTs) > 300) {
      logger.warn(`[MercadoPago Security Warning] Expired signature timestamp: ${ts}`);
    ...
    const computedHmacBuf = Buffer.from(computedHmac, 'utf8');
    const v1Buf = Buffer.from(v1, 'utf8');

    if (computedHmacBuf.length !== v1Buf.length || !crypto.timingSafeEqual(computedHmacBuf, v1Buf)) {
      logger.warn('[MercadoPago Security] Invalid signature match');
    ...
    logger.error('[MercadoPago Security Error] Signature verification exception:', { error: err.message });
